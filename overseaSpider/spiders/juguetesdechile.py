...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['juguetesdechile.com']
    start_urls = ['https://www.juguetesdechile.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath("//div[@class='nav-bounds nav-fashion-desktop-categories-popover']/div[1]//li[@class='header--custom-text']/a/@href").getall()
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[@class='ui-search-item__group ui-search-item__group--title']/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath("//span[@itemprop='offers']//span[@class='price-tag-fraction']/text()").get()
        ...
        description = response.xpath("//p[@class='ui-pdp-description__content']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = website

        images_list = response.xpath("//div[@class='ui-pdp-thumbnail__picture']/img/@src").getall()
        for i in range(len(images_list)):
            if images_list[i].find("http2")==-1:
                return
            images_list[i]=images_list[i].replace("-R.","-O.")
