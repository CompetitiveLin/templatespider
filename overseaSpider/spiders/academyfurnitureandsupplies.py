...
website = 'academyfurnitureandsupplies'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['academyfurnitureandsupplies.net']
    start_urls = ['https://academyfurnitureandsupplies.net/']
    ...
    def parse_detail(self, response):
        ...
        label = response.xpath("//div[@class='data-table']/dl/dt/text()").getall()
        if label:
            ...
            for key in s_json:
                for j in s_json[key]:
                    label1.append(s_json[key][j]["name"])
                    label2.append(s_json[key][j]["value"])
            attributes = []
            if label1:
