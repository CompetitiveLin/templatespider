...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['thepaperplacesisters.com']
    start_urls = ['https://www.thepaperplacesisters.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.thepaperplacesisters.com/legos?page=5',
                         'https://www.thepaperplacesisters.com/games?page=6',
                         'https://www.thepaperplacesisters.com/arts-and-crafts?page=2']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//li[@data-hook='product-list-grid-item']//a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        description = response.xpath("//pre[@data-hook='description']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = website

        images_list = response.xpath("//button[@data-hook='thumbnail-button']/img/@src").getall()
        if images_list:
            for i in range(len(images_list)):
                index = images_list[i].find("jpg")
                images_list[i]=images_list[i][:index]
        else:
            images_list = response.xpath("//div[@id='get-image-item-id']/@href").getall()
