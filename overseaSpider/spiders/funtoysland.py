...
class ThecrossdesignSpider(scrapy.Spider):
    ...
    def parse_detail(self, response):
        ...
        price_old = 100000000
        for key in skuAttr_json:
            if float(skuAttr_json[key]['_price'])< price_old:
                price_old = float(skuAttr_json[key]['_price'])
            if float(skuAttr_json[key]['_salePrice']) < price_new:
                price_new = float(skuAttr_json[key]['_salePrice'])
