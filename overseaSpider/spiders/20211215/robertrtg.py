...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> robertrtg
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['robertrtg.com']
    start_urls = ['https://www.robertrtg.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.robertrtg.com/store/pc/viewcategories.asp']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse1)

    def parse1(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//div[@class="pcShowCategory pcShowCategoryBgHover"]/a/@href').getall()
        if category_urls:
            for category_url in category_urls:
                category_url = response.urljoin(category_url)
                yield scrapy.Request(url=category_url, callback=self.parse1)
        else:
            category_urls1 = response.xpath('//a[@onclick="pcf_Open_viewAll();"]/@href').get()

            if category_urls1:
                category_urls1 = response.urljoin(category_urls1)
                yield scrapy.Request(url=category_urls1, callback=self.parse_list)
            else:
                yield scrapy.Request(url=response.url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//a[@class="pcButton pcButtonMoreDetails"]/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath('//span[@class="pcShowProductListPrice"]/text()').get()
        ...
        cat_list = response.xpath('//div[@class="pcPageNav"]/a/text()').getall()[1:]
        ...
        images_list = response.xpath('//div[@class="pcShowAdditional"]/a/@href').getall()
        if images_list:
            images_list = [response.urljoin(url) for url in images_list]
        else:
            images_list = [response.urljoin(response.xpath('//img[@itemprop="image"]/@src').get())]
