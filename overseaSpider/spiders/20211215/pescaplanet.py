...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> pescaplanet
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['pescaplanet.com']
    start_urls = ['https://www.pescaplanet.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//div[@class="vertitem"]/a/@href').getall()
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//a[@class="newProductsNome"]/@href').getall()[3:]
        for detail_url in detail_url_list:
            yield scrapy.Request(url=detail_url, callback=self.parse_detail)
        next_page_url = response.xpath('//a[@title=" Pagina successiva "]/@href').get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath('//span[@class="productInfoPrezzoValore"]/text()').get()
        if price:
            items["current_price"] = items["original_price"] = self.price_fliter(price)
        else:
            items["current_price"] = self.price_fliter(response.xpath('//span[@class="productInfoPrezzoScontatoValore"]/b/text()').get())
            price = response.xpath('//span[@class="productInfoPrezzoListinoValore"]/b/text()').get()
            items["original_price"] = self.price_fliter(price) if price else items["current_price"]

        name = response.xpath('//td[@class="pInfoNome"]/h1/text()').get().strip()
        items["name"] = name

        cat_list = response.xpath('//td[@class="breadcrumb"]/a/@title').getall()
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath('//div[@id="DESC"]//text()').getall()
        ...
        images_list = response.xpath('//td[@class="gallery_img smallText"]/a/@href').getall()
        items["images"] = images_list
        items["brand"] = ''

        opt_name = response.xpath('//tr[@class="prod_options"]//td[@class="main"]/b/text()').getall()[1:]
        if not opt_name:
            items["sku_list"] = []
            # return
        else:
            ...
            for i in range(opt_length):
                value_temp = response.xpath('//tr[@class="prod_options"]//tr/td/table//tr[' + str(i+2) + ']//option/text()').getall()
                if value_temp:
            ...
            for attrs in attrs_list:
                ...
                for attr in attrs.items():
                    ...
                    if '€' in attr[1]:
                        add_price = attr[1].split('€')[-1].split(')')[0]
                    else:
                        add_price = '0'
                if len(other_temp):
                    sku_attr["other"] = other_temp

                sku_info["current_price"] = str(float(items["current_price"]) + float(add_price))
                sku_info["original_price"] = str(float(items["original_price"]) + float(add_price))
