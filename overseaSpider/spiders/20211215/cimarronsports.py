...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> cimarronsports
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['cimarronsports.com']
    start_urls = ['https://www.cimarronsports.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//ul[@id="menuId3"]/li/a/@href').getall()
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse1)

    def parse1(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//ul[@class="subCategories"]/li/a/@href').getall()
        for category_url in category_urls:
    ...
    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//div[@class="productGridView"]/a/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath('//span[@class="salePrice"]/text()').get()
        ...
        cat_list = response.xpath('//div[@class="innerBreadCrumbs"]/a/text()').getall()
        ...
        images_list = response.xpath('//div[@id="thumbnailSlider"]/div/img/@src').getall()
        images_list = [img.split('medium')[0] + 'large' + img.split('medium')[1] for img in images_list]
        items["images"] = images_list
        items["brand"] = ''

        opt_name = response.xpath('//div[@id="optionSelectLabel1"]/text()').getall()
        # opt_name = []
        if not opt_name:
            items["sku_list"] = []
            # return
            ...
            opt_value = []
            # print(opt_name)
            value_temp = response.xpath('//select[@data-name="Size"]/option/@data-name').getall()[1:]
            opt_value.append(value_temp)
            choice = response.xpath('//select[@data-name="Size"]/option/@data-map').getall()
            # print(opt_value)
            attrs_list = []
            ...
            for attrs in attrs_list:
                ...
                for attr in attrs.items():
                    if attr[0] == 'Select Size':
                        sku_attr["size"] = attr[1]
                    elif attr[0] == 'Color':
                        sku_attr["colour"] = attr[1]
                    else:
                        other_temp[attr[0]] = attr[1]
                    add_price = choice[value_temp.index(attr[1])]
                if len(other_temp):
                    sku_attr["other"] = other_temp


                sku_info["current_price"] = str(convert(float(items["current_price"]) + float(add_price)))
