...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> fulton-armory
@IDE    ：PyCharm
@Author ：Mr. Tutou
@Date   ：2022/1/4 16:24
@Desc   ：
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['fulton-armory.com']
    start_urls = ['https://www.fulton-armory.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//ul[@class="module-list cat-nav"]/li/ul/li/a/@href').getall()
    ...
    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//div[@class="product-list-options"]/h5/a/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        items["current_price"] = self.price_fliter(response.xpath('//span[@class="prod-detail-cost-value"]/text()').get())
        items["original_price"] = items["current_price"]

        name = response.xpath('//div[@class="product-detail"]/h1/text()').get().strip()
        items["name"] = name

        cat_list = response.xpath('//span[@id="ctl00_breadcrumbContent"]/span/a/text()').getall()
        ...
        description = response.xpath('//div[@class="prod-detail-desc"]//text()').getall()
        ...
        images_list = response.xpath('//a[@class="MagicZoomPlus"]/@href').getall()
        images_list = [response.urljoin(url) for url in images_list]
        items["images"] = images_list
        items["brand"] = ''

        opt_name = response.xpath('//span[@class="bundles-option-label"]/text()|//span[@class="label"]/text()').getall()
        ...
        if not opt_name:
            ...
            for i in range(opt_length):
                value_temp = response.xpath('//div[@class="bundles-options"]/div[' + str(
                    i + 1) + ']//option/text()').getall()[1:]

                if not value_temp:
                    value_temp = response.xpath('//div[@class="variationDropdownPanel"]//option/text()').getall()[1:]
            ...
            for attrs in attrs_list:
                ...
                for attr in attrs.items():
                    ...
                    if '$' in attr[1]:
                        temp = attr[1].split('$')[-1].split(')')[0]
                        add_price += float(temp)
                if len(other_temp):
