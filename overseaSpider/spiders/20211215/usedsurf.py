...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> usedsurf
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['usedsurf.com']
    start_urls = ['https://www.usedsurf.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://usedsurf.com/new-surfboards/','https://usedsurf.com/used-surfboards/',\
                         'https://usedsurf.com/accessories/','https://usedsurf.com/on-sale/']
    ...
    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//h3[@class="card-title"]/a/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        items["current_price"] = self.price_fliter(response.xpath('//span[@class="price price--withoutTax"]/text()').get().strip())
        price = response.xpath('//span[@class="price price--rrp"]/text()').get().strip()
        if price:
        ...
        if not images_list:
            images_list = response.xpath('//a[@itemprop="image"]/@href').getall()
        items["images"] = images_list
        items["brand"] = ''
        for d in description:
            if d.strip().split(':')[0]=='Brand':
                items["brand"] = d.strip().split(':')[1].strip()
                break


        opt_name = response.xpath('//div[@class="productView-options"]//label[@class="form-label '
                                  'form-label--alternate form-label--inlineSmall"]/text()').getall()
        if not opt_name:
            items["sku_list"] = []
            # return
        else:
            opt_name = [name.replace(':', '').strip().split()[0] for name in opt_name if name.strip()]
            opt_value = []
            # print(opt_name)
            if opt_name == ['Size']:
                value_temp = response.xpath('//div[@class="productView-options"]//span[@class="form-option-variant"]/text()').getall()
            elif opt_name == ['Color']:
                value_temp = response.xpath('//div[@class="productView-options"]//input[@type="radio"]/@aria-label').getall()
            elif opt_name == ['Styles']:
                value_temp = response.xpath('//div[@class="productView-options"]//label[@class="form-label"]/text()').getall()
            else:
                print('sssssssssssssss',opt_name)
