...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> parafarmaciapararegalar
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
website = 'parafarmaciapararegalar'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['parafarmaciapararegalar.com']
    start_urls = ['http://parafarmaciapararegalar.com']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//div[@class="container"]//li/a/@href').getall()[:-1]
    ...
    def parse_list(self, response):
        ...
        next_page_url = response.xpath('//a[@class="next js-search-link"]/@href').get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        items["current_price"] = response.xpath('//span[@itemprop="price"]/@content').get()
        original_price = response.xpath('//span[@class="regular-price"]/text()').get()
        if original_price:
            items["original_price"] = self.price_fliter(original_price.replace(',','.'))
        else:
        ...
        description = response.xpath('//div[@id="description"]/div[@class="product-description"]//text()').getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = 'parafarmaciapararegalar.com'
        ...
        images_list = response.xpath('//div[@class="product-cover"]/img/@src').getall()
