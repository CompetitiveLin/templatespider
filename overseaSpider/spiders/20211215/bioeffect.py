...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> bioeffect
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['bioeffect.com']
    start_urls = ['https://www.bioeffect.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.bioeffect.com/products']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self,response):
        category_urls = response.xpath('//a[@class="CardLink_cardLink__fC1Q2"]/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        items["current_price"] = self.price_fliter(response.xpath('//div[@class="ProductHero_productHero__priceWrap__J1yqr"]/text()').get())
        price = response.xpath('//div[@class="ProductHero_productHero__priceWrap__J1yqr"]/span/text()').get()
        items["original_price"] = self.price_fliter(price.strip()) if price else items["current_price"]

        name = response.xpath('//h1[@class="Heading_h3__wGHrA"]/text()').get().strip()
        items["name"] = name
        ...
        description = response.xpath('//div[@class="RichText_richText__3RSgr"]//text()').getall()
        ...
        images_list = response.xpath('//div[@class="Gallery_gallery__imageWrap__lSiJX"]//img[@class="GalleryItem_galleryItem__image__LHaGe"]/@src').getall()
        items["images"] = images_list
        items["brand"] = 'BIOEFFECT'
