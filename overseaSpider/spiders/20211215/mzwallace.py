...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> mzwallace
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['mzwallace.com']
    start_urls = ['https://www.mzwallace.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.mzwallace.com/shop']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//div[@class="product-image-container"]/a/@href').getall()
        detail_url_list = [self.start_urls[0] + url[1:] for url in detail_url_list]
        for detail_url in detail_url_list:
            yield scrapy.Request(url=detail_url, callback=self.parse_detail)
        next_page_url = response.xpath('//a[@class="load-more-button"]/@href').get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        name = response.xpath('//h1[@data-test="Product Name"]/text()').get().strip()
        items["name"] = name
        ...
        description = response.xpath('//div[@class="product-info-content"]//text()').getall()
        ...
        images_list = response.xpath('//link[@itemprop="image"]/@href').getall()
        items["images"] = images_list
        items["brand"] = 'MZWALLACE'
        ...
        if not opt_name:
            ...
            opt_value = []
            # print(opt_name)
            urls = response.xpath('//ul[@class="swatches color"]/li/a/@href').getall()
            urls = [self.start_urls[0] + url[1:] for url in urls]
            value_temp1 = []
            for url in urls:
                html = requests.get(url)
                selector = Selector(text=html.text)
                value_temp1.append(selector.xpath('//meta[@itemprop="price"]/@content').get())

            value_temp = response.xpath('//ul[@class="swatches color"]/li/a/@aria-label').getall()
            ...
            for attrs in attrs_list:
                ...
                for attr in attrs.items():
                    ...
                    new_price = value_temp1[value_temp.index(attr[1])]
                if len(other_temp):
