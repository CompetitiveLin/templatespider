...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> 58products
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['58products.com']
    start_urls = ['https://en.58products.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://en.58products.com/shop/ceelings/','https://en.58products.com/shop/tassen/',\
                         'https://en.58products.com/shop/talent/','https://en.58products.com/shop/cannics/']
        for category_url in category_urls:
            cat = category_url.split('/')[-2]
            yield scrapy.Request(url=category_url, callback=self.parse_list, meta={'cat': cat})

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//a[@class="noLinkDecoration CustomProduct Widget SingleTeaser  "]/@href').getall()
        detail_url_list = [self.start_urls[0] + url1[1:] for url1 in detail_url_list]
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath('//div[@class="secondColumn column"]//div[@class="BasicText lineThrough"]/text()').get()
        items["original_price"] = self.price_fliter(price) if price else items["current_price"]


        name = response.xpath('//div[@class="secondColumn column"]/h3/text()').get().strip()
        items["name"] = name
        if name == 'Gift card': return
        cat_list = response.xpath('//div[@class="CustomNavigationPath"]/a/text()').getall()
        cat_list[-1] = response.meta.get('cat')
        ...
        description = response.xpath('//div[@class="BasicText Widget  content"]/p//text()').getall()
        ...
        images_list = response.xpath('//div[@class="stage translateCss"]//div/img/@src').getall()
        images_list = [self.start_urls[0] + url[1:] for url in images_list]
        items["images"] = images_list
        items["brand"] = 'Fiftyeight'

        opt_name = response.xpath('//div[@class="textColumn"]/text()').get()
        if not opt_name:
            items["sku_list"] = []
            # return
        else:
            ...
            opt_value = []
            # print(opt_name)
            value_temp = response.xpath('//div[@class="secondColumn column"]//img/@title').getall()
            if value_temp:
