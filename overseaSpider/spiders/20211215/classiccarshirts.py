...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> classiccarshirts
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['classiccarshirts.com']
    start_urls = ['https://www.classiccarshirts.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//li[@class="vnav__item"]/a/@href').getall()
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//a[@class="v-product__img"]/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        items["source"] = 'classiccarshirts.com'
        ...
        opt_name = response.xpath('//table[@id="options_table"]//td[@align="right"]/text()').getall()
        opt_name1 = []
        for opname in opt_name:
            if opname in ['\r\nSize','\r\nColor','\r\nDesign Placement']:
                opt_name1.append(opname.split('\n')[-1])
        opt_name = opt_name1
        if not opt_name:
            items["sku_list"] = []
            # return
            ...
            if opt_length:
                opt_value.append(response.xpath('//select[@title="Size"]/option/text()').getall()[1:])
                opt_value.append(response.xpath('//select[@title="Color"]/option/text()').getall()[1:])
                opt_value.append(response.xpath('//select[@title="Design Placement"]/option/text()').getall()[1:])
            # print(opt_value)
            attrs_list = []
            ...
            for attrs in attrs_list:
                ...
                add_price = float(items["original_price"])
                try:
                    add_price += float(sku_attr["size"].split('$')[-1].split(']')[0])
                except:
                    add_price = add_price
                try:
                    add_price += float(sku_attr["colour"].split('$')[-1].split(']')[0])
                except:
                    add_price = add_price
                sku_info["current_price"] = str(add_price).split('0')[0]
