...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> credomen
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['credomen.com']
    start_urls = ['https://www.credomen.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.credomen.com/shoes.html','https://www.credomen.com/clothing.html','https://www.credomen.com/accessories.html']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//figure[@class="itemImg"]/a/@href').getall()
        detail_url_list = [self.start_urls[0] + url[1:] for url in detail_url_list]
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath('//span[@class="newPrice price liquid"]/text()').get()
        if price:
            price = self.price_fliter(price)
        else:
            price = response.xpath('//span[@itemprop="price"]/@value').get()
        items["current_price"] = price
        price1 = response.xpath('//div[@class="productRight"]//span[@class="oldPrice"]/text()').get()
        items["original_price"] = self.price_fliter(price1) if price1 else items["current_price"]

        name1 = response.xpath('//header[@id="itemName"]//span/text()').getall()
        name = ''
        for temp in name1:
            name = name + " " + temp
        items["name"] = name.strip()

        cat_list = response.xpath('//div[@id="breadcrumbs"]//a/@title').getall()
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath('//div[@class="acContent"]//p/text()').getall()
        ...
        images_list = response.xpath('//li[@class="jq-popup-zoom liquid"]/img/@src').getall()
        images_list = [self.start_urls[0] + url[1:] for url in images_list]
        items["images"] = images_list
        items["brand"] = response.xpath('//span[@class="bname"]/text()').get()

        opt_name = response.xpath('//span[@class="title"]/text()').getall()
        if not opt_name:
            items["sku_list"] = []
            # return
        else:
            ...
            opt_value = []
            # print(opt_name)
            if opt_name[0] == 'Select your size':
                opt_name[0] = 'Size'
            value_temp = response.xpath('//div[@class="kenWrap"]//a/@title').getall()
            if len(value_temp) > 1:
