...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> eastbayteamsales
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['eastbayteamsales.com']
    start_urls = ['https://www.eastbayteamsales.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.eastbayteamsales.com/Mens/_-_/N-1p?crumbs=61','https://www.eastbayteamsales.com/Womens/_-_/N-1q?crumbs=62',\
                         'https://www.eastbayteamsales.com/Boys/_-_/N-1my?crumbs=2122','https://www.eastbayteamsales.com/Girls/_-_/N-1mz?crumbs=2123']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//a[@class="quickviewEnabled"]/@href').getall()
        for detail_url in detail_url_list:
            yield scrapy.Request(url=detail_url, callback=self.parse_detail)
        next_page_url = response.xpath('//a[@class="next"]/@href').get()
        if next_page_url:
            next_page_url = self.start_urls[0] + next_page_url[1:]
    ...
    def parse_detail(self, response):
        ...
        if response.url != 'https://www.eastbayteamsales.com':

            price = response.xpath('//span[@class="current_price"]/text()').get()
            ...
            images_list = response.xpath('//div[@id="pdp_styles"]/a/img/@src').getall()
            images_list = ['https:'+ img.split('cart')[0] + 'large_wide' + img.split('cart')[1] for img in images_list]
            ...
            if brand:
                ...
                items["brand"] = ''
            opt_name = [response.xpath('//div[@id="pdp_info"]/strong/text()').get()]
            if not opt_name:
                items["sku_list"] = []
                # return
            else:
                ...
                for i in range(opt_length):
                    value_temp = response.xpath('//div[@id="pdp_styles"]/a/@title').getall()
                    if value_temp:
