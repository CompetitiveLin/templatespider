...
class ThecrossdesignSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        cat_name = response.xpath('//a[@class="_27Yy0"]/div/div/p/text()').getall()
        for i in range(len(category_urls)):
            if category_urls[i] == "https://www.naptoyshop.com/toy-shop":
                continue
            yield scrapy.Request(url=category_urls[i], callback=self.parse_list, meta={"cat_name":cat_name[i]})
    ...
    def parse_detail(self, response):
        ...
        items["cat"] = name
        items["detail_cat"] = '{}/{}/{}'.format('Home',cat_name_1,name)
