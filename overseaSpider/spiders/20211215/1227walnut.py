...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> 1227walnut
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['1227walnut.com']
    start_urls = ['https://www.1227walnut.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.1227walnut.com/adidas-c-1/']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//div[@class="centerBoxContentsProducts centeredContent back"]/a/@href').getall()
    ...
    def parse_detail(self, response):
        ...
        items["current_price"] = self.price_fliter(response.xpath('//h2[@id="productPrices"]/span[@class="productSpecialPrice"]/text()').get())
        price = response.xpath('//h2[@id="productPrices"]/span[@class="normalprice"]/text()').get()
        items["original_price"] = self.price_fliter(price) if price else items["current_price"]

        name = response.xpath('//h1[@id="productName"]/text()').get().strip()
        items["name"] = name

        cat_list = response.xpath('//div[@id="navBreadCrumb"]/a/text()').getall()
        ...
        images_list = response.xpath('//div[@style="text-align:center;"]/img/@src').getall()
        images_list = [self.start_urls[0]+ url for url in images_list]
        items["images"] = images_list
        items["brand"] = 'adidas'

        opt_name = response.xpath('//label[@class="attribsSelect"]/text()').getall()
        if not opt_name:
            items["sku_list"] = []
            # return
        else:
            ...
            opt_value = []
            # print(opt_name)
            value_temp = response.xpath('//div[@class="back"]//option/text()').getall()[1:]
            if value_temp:
