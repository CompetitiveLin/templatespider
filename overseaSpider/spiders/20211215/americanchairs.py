...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> americanchairs
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['americanchairs.com']
    start_urls = ['https://www.americanchairs.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath('//a[@class="subnavlit"]/@href').getall()[:9]
        category_urls = [response.urljoin(url) for url in category_urls]
        category_urls.append('https://www.americanchairs.com/sports-logo-bar-stools.html')
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath('//b/a/@href').getall()
        if not detail_url_list:
            detail_url_list = response.xpath('//font/a/@href').getall()
            detail_url_list = [response.urljoin(url) for url in detail_url_list]
            for detail_url in detail_url_list:
                yield scrapy.Request(url=detail_url, callback=self.parse_list)
    ...
    def parse_detail(self, response):
        ...
        price = response.xpath('//img[@align="absmiddle"]/@alt').get()
        if not price:
            return
        ...
        cat_list = response.xpath('//tr[@valign="top"]/td[3]/a/text()').getall()
        ...
        images_list = response.xpath('//div[@itemtype="http://schema.org/Product"]//img/@src').getall()[:1]
        items["images"] = images_list
        items["brand"] = ''

        opt_name = response.xpath('//form[@class="atcform"]/text()').getall()
        opt_name = [opt for opt in opt_name if '(' in opt]
        if not opt_name:
            items["sku_list"] = []
            # return
        else:
            ...
            for i in range(opt_length):
                value_temp = response.xpath('//form[@class="atcform"]/select[' + str(
                    i + 1) + ']/option/text()').getall()[1:]
                value_temp = [temp for temp in value_temp if temp.strip() and not ':' in temp]
                if value_temp:
            ...
            for attrs in attrs_list:
                ...
                for attr in attrs.items():
                    ...
                    if '$' in attr[1]:
                        add_price += float(attr[1].split('$')[-1].split(')')[0])
                if len(other_temp):
                    sku_attr["other"] = other_temp

                sku_info["current_price"] = str(float(items["current_price"]) + add_price)
                sku_info["original_price"] = str(float(items["original_price"]) + add_price)
