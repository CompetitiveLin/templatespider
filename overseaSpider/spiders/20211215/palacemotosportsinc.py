...
# -*- coding: UTF-8 -*-
'''=================================================
@Project -> File   ：templatespider -> palacemotosportsinc
@IDE    ：PyCharm
@Author ：Mr. Tutou
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['palacemotosportsinc.com']
    start_urls = ['https://www.palacemotosportsinc.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.palacemotosportsinc.com/new-models/can-am-98','https://www.palacemotosportsinc.com/new-models/ski-doo-178',\
                         'https://www.palacemotosportsinc.com/new-models/yamaha-40','https://www.palacemotosportsinc.com/new-models/yamaha-273']
        gurl = 'https://www.palacemotosportsinc.com/new-models/'
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse1,meta={'u':gurl})

    def parse1(self, response):
        """获取全部分类"""
        url1 = response.meta.get('u')
        category_urls = response.xpath('//h3[@class="showcase-header"]/a/@href').getall()
        category_urls = [url1 + urls for urls in category_urls]
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list,meta={'u':url1})

    def parse_list(self, response):
        """商品列表页"""
        url1 = response.meta.get('u')
        detail_url_list = response.xpath('//h3[@class="text-center showcase-header"]/a/@href').getall()
        detail_url_list = [url1 + url for url in detail_url_list]
    ...
    def parse_detail(self, response):
        ...
        cat_list = response.xpath('//span[@itemprop="name"]/text()').getall()
        if cat_list:
        ...
        attr1_list = response.xpath('//div[@class="row"]/div[@class="col-xs-12 col-lg-5 attr-name"]/strong/text()').getall()
        attr2_list = response.xpath('//div[@class="row"]/div[@class="col-xs-12 col-lg-7 attr-value"]/text()').getall()
        attribute = []
        for a in range(len(attr1_list)):
            attribute.append(attr1_list[a]+":"+attr2_list[a])
        items["attributes"] = attribute

        images_list = response.xpath('//div[@class="unit-image-container"]/a/@href|//div[@class="unit-image-container "]/a/@href|//div[@class="unit-image-container hidden"]/a/@href'\
                                     ).getall()
