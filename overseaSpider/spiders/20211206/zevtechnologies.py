...
website = 'zevtechnologies'
website_url = 'https://www.zevtechnologies.com'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['zevtechnologies.com']
    start_urls = ['https://www.zevtechnologies.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.zevtechnologies.com/Shop']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[@class='facets-item-cell-grid-image-wrapper']/a/@href").getall()
        for detail_url in detail_url_list:
            detail_url = website_url + detail_url
            yield scrapy.Request(url=detail_url, callback=self.parse_detail)
        next_page_url = response.xpath("//li[@class='global-views-pagination-next']/a/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        price_new = response.xpath("//form//div[@class='product-views-price']//meta[@itemprop='price']/@content").get()
        price_new = str(price_new)
        price_old = response.xpath("//form//div[@class='product-views-price']//small[@class='product-views-price-old']/text()").get()
        if price_old:
        ...
        items["name"] = name
        items["brand"] = 'ZEV Technologies'
        cat_list = response.xpath("//div[@class='global-views-breadcrumb-interior']/li/a/text()").getall()
        if cat_list:
        ...
        # images_list_tmp = response.xpath("//div[@class='item-relations-related-item-thumbnail-wrapper']/img/@src").get()
        images_list= response.xpath("//ul[@class='bxslider']/li//div[@class='product-details-image-gallery-container-wrapper']//img/@src").getall()
        # images_list = []
        # if images_list_tmp:
        #     images_list.append(images_list_tmp)
        #     items["images"] = images_list
        # else:
        #     images_list.append(images_list_tmp1)
        items["images"] = images_list
