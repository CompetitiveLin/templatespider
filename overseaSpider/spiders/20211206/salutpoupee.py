...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['salutpoupee.com']
    start_urls = ['https://www.salutpoupee.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://www.salutpoupee.com/fr/3-poupees']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[contains(@class,'pro_first_box')]/a/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        images_list = response.xpath("//div[@class='swiper-wrapper']/div[contains(@class,'swiper-slide')]//img/@data-srcset").getall()
        images_list_tmp = []
        for i in range(len(images_list)):
            images_list[i]=images_list[i].replace(" 2x",'')
            if images_list[i].find("small")==-1:
                images_list_tmp.append(images_list[i])

        items["images"] = images_list_tmp
