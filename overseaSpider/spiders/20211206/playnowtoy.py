...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['playnowtoy.com']
    start_urls = ['https://www.playnowtoy.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath("//li[@class='item sub']/ul[@class='subnav']/li[@class='subitem']/a/@href").getall()
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[@class='image-wrap']/a/@href").getall()
        for detail_url in detail_url_list:
            yield scrapy.Request(url=detail_url, callback=self.parse_detail)
        next_page_url = response.xpath("//li[@class='next enabled']/a[@title='Next page']/@href").get()
        if next_page_url:
    ...
    def parse_detail(self, response):
        ...
        cat_list = response.xpath("//div[@role='nav']/a/text()").getall()
        if cat_list:
            cat_list = [cat.strip() for cat in cat_list if cat.strip()]
            items["cat"] = cat_list[-1]
            items["detail_cat"] = '/'.join(cat_list)

        description = response.xpath("//div[@class='tabsPages']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = website

        images_list = response.xpath("//div[@class='thumbs row']//a[@data-image-id]/img/@src").getall()
        for i in range(len(images_list)):
            images_list[i]=images_list[i].replace("156x230x2","1600x2048x2")
