...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['kruszki.com']
    start_urls = ['https://kruszki.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://kruszki.com/kategoria/zabawki-drewniane',
                         'https://kruszki.com/kategoria/przytulanki',
                         'https://kruszki.com/kategoria/dekoracje',
                         'https://kruszki.com/pl/c/Outlet/52']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list)

    def parse_list(self, response):
        """商品列表页"""
        detail_url_list = response.xpath("//div[@class='product-inner-wrap']/a[@rel]/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        price_new = response.xpath("//div[@class='price']/em/text()").get()
        price_old = response.xpath("//div[@class='price']/del/text()").get()
        price_new = price_new.replace(',','.')
        price_new = re.match(r"\d+\.\d+",price_new).group()
        if price_old:
            price_old = price_old.replace(',', '.')
            price_old = re.match(r"\d+\.\d+", price_old).group()
        ...
        cat_list = response.xpath("//ul[@itemscope]/li[position()>1]//span[not(contains(@class,'raq'))]/text()").getall()
        if cat_list:
        ...
        images_list = response.xpath("//ul[@class='r--l-flex r--l-flex-wrap']/li/a/@href").getall()
        if not images_list:
            images_list=response.xpath("//div[@class='mainimg productdetailsimgsize row']//img/@src").getall()
