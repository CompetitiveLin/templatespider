...
website = 'calphalon'
website_url = 'https://www.calphalon.com/'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['calphalon.com']
    start_urls = ['https://www.calphalon.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath("//div[@class='text14 bold l2-heading d-none d-lg-flex'][1]/a[contains(@data-category-name,'Shop All')]/@href").getall()
        print(category_urls)
        for index, category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_list, meta={'url', category_url})

    def parse_list(self, response):
        """商品列表页"""
        next_page_url = response.xpath("//input[@class='append-params']/@value").get()
        if next_page_url:
            Url = response.meta.get('url')
            next_page_url = Url + '?' + next_page_url
            yield scrapy.Request(url=next_page_url, callback=self.parse_list,meta={'url',Url} )
        else:
            detail_url_list = response.xpath("//a[@class='product-tile-link']/@href").getall()
    ...
    def parse_detail(self, response):
        ...
        for label in label_list:
            ...
            if 'size' in key:
                ...
                size_list = [{'id': v.xpath('./@data-product-attribute-value').get(),
                              'value': v.xpath('./span[1]/text()').get().strip()} for v in values]
