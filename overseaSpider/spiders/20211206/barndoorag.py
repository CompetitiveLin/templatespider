...
website = 'barndoorag'
website_url = 'https://barndoorag.com'
...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['barndoorag.com']
    start_urls = ['https://barndoorag.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = ['https://barndoorag.com/brands']
        for category_url in category_urls:
            yield scrapy.Request(url=category_url, callback=self.parse_brand)

    def parse_brand(self,response):
        brand_urls = response.xpath("//li[@class='brand']/article/figure/a/@href").getall()
        # brand_urls = ['https://barndoorag.com/leon-boots/']
        for brand_url in brand_urls:
            yield scrapy.Request(url=brand_url, callback=self.parse_list)
        ...
        if next_page_url:
            ...
            yield scrapy.Request(url=next_page_url, callback=self.parse_brand)
    ...
    def parse_detail(self, response):
        ...
        opt_name_tmp = response.xpath("//div[@data-product-option-change]/div[1]/label/text()").getall()
        opt_name = []
        ...
        if not opt_name:
            ...
            for i in range(opt_length):
                value_temp = response.xpath("//div[@data-product-option-change]/div["+str(i+1)+"]/select/option[@data-product-attribute-value]/text()").getall()
                if value_temp:
