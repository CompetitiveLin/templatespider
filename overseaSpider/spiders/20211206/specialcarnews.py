...
class ThecrossdesignSpider(scrapy.Spider):
    name = website
    allowed_domains = ['specialcarnews.com']
    start_urls = ['https://www.specialcarnews.com/']
    ...
    def parse(self, response):
        """获取全部分类"""
        category_urls = response.xpath("//li[contains(@class,'level0')]/a/@href").getall()
    ...
    def parse_list(self, response):
        ...
        page_str = response.xpath("//h5/span/text()").get()
        page_number=page_str.replace('(','')
        page_number=page_number.replace(')','')
        page_number=page_number.replace(',','')
        page=math.ceil(int(page_number)/48)
        for i in range(page):
            next_page_url = base_url + '?p=' + str(i + 1)
    ...
    def parse_detail(self, response):
        ...
        price_now = response.xpath("//div[@class='product-title-price']/div[@class='price-box']/span[@class='special-price']/span/text()").get()
        price_old = response.xpath("//div[@class='product-title-price']/div[@class='price-box']/span[@class='old-price']/span/text()").get()
        ...
        if price_old is None:
            items["original_price"] = price_now
        else:
            items["original_price"] = price_old
        items["current_price"] = price_now

        name = response.xpath("//h1[@class='product-fullname']/text()").get()
        items["name"] = name
        ...
        description = response.xpath("//div[@class='product-details']").getall()
        items["description"] = self.filter_text(self.filter_html_label(''.join(description)))
        items["source"] = website

        images_list = response.xpath("//li/a/img[@class='img-responsive']/@src").getall()
